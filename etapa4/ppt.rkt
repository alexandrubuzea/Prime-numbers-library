...
;; the functional way is the right way <3
(define (dot-product X Y)
  (foldl + 0 (map * X Y))
  )

(define (multiply M V)
  (foldr cons '() (map (λ(L) (dot-product L V)) M))
  )
...
(define (gen-next l) (map (λ(func) (func l)) (map (λ(matrix) ((curry multiply) matrix)) (list T1 T2 T3))))
...
(define ppt-stream-in-tree-order
  (let build ([result empty-stream] [queue (stream-cons '(3 4 5) empty-stream)])
  (let* ([initial (stream-first queue)] [next-tr (gen-next initial)] [next-tr-str (list->stream next-tr)])
  (stream-cons initial (build (stream-append result next-tr-str) (stream-append (stream-rest queue) next-tr-str))))
  ))
...
(define (stream-take-nth str n)
  (if (= n 1)
      (stream-first str)
      (stream-take-nth (stream-rest str) (- n 1))))

(define (pairs G H)
  (let build ([i 1] [j 1])
    (let* ([elem-ith (stream-take-nth G i)] [elem-jth (stream-take-nth H j)])
      (stream-cons (cons elem-ith elem-jth) (if (= i j)
                                                (build 1 (+ j 1))
                                                (build (+ i 1) j)
                                                )
      )
...
(define (naturals-from-n n)
  (stream-cons n (naturals-from-n (+ n 1)))
  )

(define naturals (naturals-from-n 1))

(define gh-pairs-stream
  (let ([odd-numbers (stream-map (λ(elem) (- (* 2 elem) 1)) naturals)])
  (stream-filter (λ(pair) (if (= 1 (gcd (car pair) (cdr pair))) #t #f)) (pairs odd-numbers (stream-rest odd-numbers))
                 )
  )
  )
...
;; We use stream-map ofc <3

(define ppt-stream-in-pair-order
  (stream-map (λ(p)
                (let* ([g (car p)] [h (cdr p)] [a (* g h)] [b (/ (- (sqr h) (sqr g)) 2)] [c (/ (+ (sqr h) (sqr g)) 2)])
                       (list a b c)
                       )
                ) gh-pairs-stream)
  )
