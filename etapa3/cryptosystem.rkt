...
(define (key n)
  (let* ([quadruple (get-nth-quadruple n)] [e (cadr quadruple)] [f (caddr quadruple)] [operations (list a1 b1 c1 a2 b2 c2 a3 b3 c3)])
    (map (compose (λ(n) (modulo n 27)) (λ(func) (func e f))) operations)
    )
  )
...
(define (message->codes message)
  (let ([my-string (string->list message)])
    (map (λ(ch) (if (equal? ch #\space) 0 (- (char->integer ch) (- (char->integer #\a) 1)))
           ) my-string)
    )
  )
...
(define (codes->message codes)
  (list->string (map (λ(code) (if (= code 0) #\space (integer->char (+ ( - (char->integer #\a) 1) code)))) codes))
  )
...
(define (extend-key key size)
  (let* ([key-size (length key)] [quot (+ 1 (quotient size key-size))] [repeat ((curry append) key)] [domain (
                                                                                                              let generate ([n quot] [result '()])
                                                                                                               (if (= n 0) result
                                                                                                                   (generate (- n 1) (repeat result)))
                                                                                                               )])
    (take domain size)
    )
  )
...
(define (do-my-codes message key op)
  (let* ([len-message (length message)] [extended-key (extend-key key len-message)])
    (map (compose (λ(x) (modulo x 27)) op) message extended-key)
    )
  )

(define (encrypt-codes message key)
  (do-my-codes message key +)
  )
